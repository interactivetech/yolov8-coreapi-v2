...
class BaseTrainer:
    ...
    def _do_train_inner(self,world_size=1,core_context=None):
        '''
        '''
        for op in core_context.searcher.operations():
            ...
            for epoch in range(self.start_epoch, self.epochs):
                ...
                if RANK in (-1, 0):
                    ...
                    val_dict = {}
                    val_dict.update(self.metrics)
                    for i, c in enumerate(self.validator.metrics.ap_class_index):
                        val_dict[f"{self.validator.names[c]}_P"]=self.validator.metrics.class_result(i)[0]
                        val_dict[f"{self.validator.names[c]}_R"]=self.validator.metrics.class_result(i)[1]
                        val_dict[f"{self.validator.names[c]}_mAP50"]=self.validator.metrics.class_result(i)[2]
                        val_dict[f"{self.validator.names[c]}_mAP"]=self.validator.metrics.class_result(i)[3]
    ...
    def _do_train(self, world_size=1):
        ...
        if RANK != -1:
            ...
            with det.core.init(distributed=distributed) as core_context:
                self._do_train_inner(world_size,core_context)
        else:
